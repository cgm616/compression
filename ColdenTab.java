...
public class ColdenTab {
	...
	* assign values and create the layout of the GUI.
	* @param stage is the only parameter, it comes 
	* from the Main class to provide a background for
	* the rest of the javafx content.
	* @return Nothing.
	*/
    public void makeGUI(Stage stage) {
        ...
        this.grid.setPadding(new Insets(10, 10, 10, 10)); // grid is contained by 10 pixels of empty space on all 4 sides
		...
		GridPane.setConstraints(this.clearLog, 1, 7);
        ...
        this.inputButton.setOnAction(new EventHandler<ActionEvent>() {
            @Override
			/**
			* This method creates a FileChooser to allow the user
			* to select an input file from File Explorer.
			* @param e is the only parameter, lets the method know
			* why it's being called.(the button has been clicked)
			* @return nothing.
			*/
        ...
        this.outputButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			/**
			* This method creates a FileChooser to allow the user
			* to select an output file from File Explorer.
			* @param e is the only parameter, lets the method know
			* why it's being called.(the button has been clicked)
			* @return nothing.
			*/
        ...
        this.runButton.setOnAction(new EventHandler<ActionEvent>() {
            @Override
			/**
			* This method calls run(), which compresses/expands the
			* input file in the classes Compress and Expand.
			* @param e is the only parameter, lets the method know
			* why it's being called.(the button has been clicked)
			* @return nothing.
			*/
        ...
        this.clearInputButton.setOnAction(new EventHandler<ActionEvent>() {
            @Override
			/**
			* This method clears the content of the inputText
			* text field.
			* @param e is the only parameter, lets the method know
			* why it's being called.(the button has been clicked)
			* @return nothing.
        ...
        this.clearOutputButton.setOnAction(new EventHandler<ActionEvent>() {
            @Override
			/**
			* This method clears the content of the outputText
			* text field.
			* @param e is the only parameter, lets the method know
			* why it's being called.(the button has been clicked)
			* @return nothing.
	...
	/**
	* This method is just a stub; it will be
	* overridden in subclasses Compress and
	* Expand
	* @param none.
	* @return nothing.
	*/
	...
	/**
	* Returns the GridPane field to display 
	* on-screen in Main.
	* @param none.
	* @return Node - This returns the grid 
	* field.
	*/
    public Node render() {
        return this.grid;
	...
	/**
	* Prints any input messages to the console 
	* and adds them to the log.
	* @param message is the first parameter, and
	* is the content of the message to print.
	* @param prio is the second, specifies the
	* type of message being printed.
	* @return nothing.
	*/
	...
	/**
	* Gets the fileChooser field to select an
	* input file.
	* @param none.
	* @return FileChooser - returns this object's
	* fileChooser field
	*/
